...
from ira.interfaces.mcp.server import app as mcp_server
...
def create_http_app(token: str | None = None) -> Starlette:
    if token is None:
        token = os.environ.get("IRA_MCP_TOKEN", "")
        token_file = os.environ.get("IRA_MCP_TOKEN_FILE", "")
        if not token and token_file and Path(token_file).is_file():
            token = Path(token_file).read_text().strip()
    if not token:
        raise RuntimeError("IRA_MCP_TOKEN is required for the HTTP MCP transport")
...
def run() -> None:
    host = os.environ.get("IRA_MCP_HOST", "127.0.0.1")
    port = int(os.environ.get("IRA_MCP_PORT", "8000"))
