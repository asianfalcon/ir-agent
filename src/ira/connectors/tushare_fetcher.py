...
MARKET_DIR = _SETTINGS.external_source_root / "market" / "tushare"
FIN_DIR = _SETTINGS.external_source_root / "financials" / "tushare"
NEWS_DIR = _SETTINGS.external_source_root / "news" / "tushare"
...
def _pro():
    ...
    token_file = os.environ.get("TUSHARE_TOKEN_FILE", "")
...
def fetch_financials(ts_code: str) -> int:
    ...
    try:
        ...
        data = {"income": inc.to_dict(orient="records"), "balancesheet": bal.to_dict(orient="records")}
...
def fetch_news(keywords: list[str], limit: int = 20) -> list[dict]:
    ...
    try:
        ...
        filtered = [
            ...
            if any(k in (r.get("title", "") + r.get("content", "")).lower() for k in kw_lower)
        ][:limit]
