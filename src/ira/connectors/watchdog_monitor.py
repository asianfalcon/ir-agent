"""
Local file watchdog — monitors data/inputs/ for new PDFs and triggers processing.
...
def _already_processed(conn: sqlite3.Connection, path: Path) -> bool:
    row = conn.execute(
        "SELECT mtime FROM sys_file_log WHERE file_path=?", (str(path),)
    ).fetchone()
    if row is None:
        return False
...
def _process_worker(queue: Queue) -> None:
    ...
    from ira.pipelines.text_processor import process_file  # lazy import
    from ira.connectors.acecamp.expert_processor import process_article  # lazy import
...
def start() -> None:
    ...
    print(f"[watchdog] watching {[str(d) for d in WATCH_DIRS]}")
