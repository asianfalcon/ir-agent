...
def _fetch_market(ticker: str) -> None:
    ...
    out = _SETTINGS.external_source_root / "market" / "akshare" / f"{today}_{ticker}.json"
...
def _fetch_financials(ticker: str) -> None:
    ...
    out = _SETTINGS.external_source_root / "financials" / "akshare" / f"{ticker}_{period}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    if out.exists():
        return
    try:
...
def _job_tushare_daily() -> None:
    from ira.connectors.tushare_fetcher import fetch_daily
...
def _job_tushare_news() -> None:
    from ira.connectors.tushare_fetcher import fetch_news
...
def _job_yfinance_macro() -> None:
    from ira.connectors.yfinance_fetcher import fetch_macro
    fetch_macro()
