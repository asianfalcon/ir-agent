...
from ira.interfaces.mcp.http_server import create_http_app
...
def test_http_transport_reads_token_file(tmp_path, monkeypatch):
    token_file = tmp_path / "ira_mcp_token"
    token_file.write_text("file-secret\n")
    monkeypatch.delenv("IRA_MCP_TOKEN", raising=False)
    monkeypatch.setenv("IRA_MCP_TOKEN_FILE", str(token_file))
