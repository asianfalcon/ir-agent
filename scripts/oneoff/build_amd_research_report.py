...
def add_text(
    doc, text, size=10.5, color=INK, bold=False, italic=False, before=0, after=6, align=None, keep_next=False
):
    p = doc.add_paragraph()
...
def add_table(doc, headers, rows, widths, aligns=None, font_size=8.8, zebra=True):
    ...
    for i, row_data in enumerate(rows):
        ...
        for j, value in enumerate(row_data):
            ...
            p = cell.paragraphs[0]
            alignment = (
                aligns[j] if aligns else (WD_ALIGN_PARAGRAPH.LEFT if j == 0 else WD_ALIGN_PARAGRAPH.CENTER)
            )
...
def build():
    ...
    add_text(
        doc, "超威半导体（AMD）", size=29, color=NAVY, bold=True, after=6, align=WD_ALIGN_PARAGRAPH.CENTER
    )
    add_text(
        doc,
        "AI算力第二供应链的真实弹性与兑现门槛",
    ...
    cover_points = [
        ...
        (
            "主要分歧：",
            "GW协议代表多年期潜在需求，不等于当期确认收入；权证、战略投资与客户集中降低每股价值确定性。",
        ),
        (
            "估值判断：",
            "现价并不便宜。只有在2027年收入接近1000亿美元、摊薄后EPS接近18美元时，600美元以上目标才成立。",
        ),
    ]
    for label, text in cover_points:
        add_rich_para(
            doc,
            [
                (label, {"size": 10.3, "bold": True, "color": BLUE}),
                (text, {"size": 10.3, "bold": False, "color": INK}),
            ],
            after=6,
        )
    add_source(
        doc,
        "资料来源：AMD公司公告、AMD 2026Q1财务报告、AMD Advancing AI 2026、UBS（2026-06-24）、公开市场数据；本报告测算。",
    )
    ...
    for title, body in [
        (
            "第一层｜CPU份额与ASP",
            "EPYC持续受益于核心数、能效与x86软件兼容性。Agentic AI提高控制面、检索、数据处理和传统软件负载，CPU需求有独立增量，而不仅是GPU附属。",
        ),
        (
            "第二层｜GPU与机架",
            "OpenAI、Meta、Anthropic及微软的合作把MI450从产品路线图推进到客户部署阶段。真正决定收入的是机架物料价值、交付节奏和客户验收。",
        ),
        (
            "第三层｜开放软件",
            "ROCm与Coding Agent可以降低迁移成本，但短期不能等同于CUDA生态被替代。生产稳定性、算子覆盖、多机通信和开发工具仍需持续验证。",
        ),
    ]:
        add_rich_para(
            doc,
            [
                (title, {"size": 10.4, "bold": True, "color": BLUE}),
                ("  " + body, {"size": 10.4, "bold": False, "color": INK}),
            ],
            after=8,
        )
    ...
    add_text(
        doc,
        "AMD给出的2030年AI芯片TAM约1.4万亿美元、数据中心CPU TAM约2200亿美元，说明行业空间足够大；但TAM不是收入预测。研究模型必须回答AMD在每一代产品中的单位份额、ASP、机架价值量和收入确认时点。",
    )
    add_callout(
        doc,
        "判断",
        "“市场足够大”只解决天花板问题；“客户何时验收、AMD确认多少收入、付出多少稀释和投资”才决定每股价值。",
        fill="FFF7E6",
        accent=GOLD,
    )
    ...
    add_table(
        ...
        [
            [
                ...
                "多年期6GW",
                "首个1GW自2026H2开始",
                "最高1.6亿股权证；技术、采购及股价条件",
                "2026仅计部分出货，2027-30分期",
            ],
            [
                "Meta",
                "最高6GW",
        ...
        [1280, 1400, 1900, 2720, 2060],
        font_size=8.1,
    )
    add_source(
        doc,
        "资料来源：AMD关于OpenAI、Meta、Anthropic、Microsoft和Oracle的官方公告；规模均为潜在/计划部署口径。",
    )

    add_heading(doc, "为什么不能用“美元/GW”直接乘", 2)
    add_text(
        doc,
        "一个GW可对应不同GPU数量、功耗配置、网络方案与系统边界。若AMD只确认GPU和部分网络收入，收入价值量较低而毛利率较高；若确认完整机架收入，收入更高但包含更多低毛利率部件。缺少合同结构时，收入和毛利率必须成对假设。",
    )
    add_table(
        doc,
    ...
    add_text(
        doc,
        "OpenAI与Meta的潜在规模足以显著改变AMD收入结构，也会提高单一客户、融资和项目节奏风险。AMD对NeoCloud伙伴的投资或担保有助于需求形成，但同时把供应商风险扩展为投资、信用和回收风险。",
    )
    ...
    add_source(
        doc,
        "注：2026E采用约13%的Non-GAAP税率；2027-28包含阶段性权证摊薄。由于归属条件复杂，实际股数可能显著偏离。",
    )

    add_heading(doc, "毛利率的核心会计分叉", 2)
    add_text(
        doc,
        "MI450与Helios规模化会提升数据中心占比，但不保证毛利率同步上升。若AMD确认更多机架级硬件收入，收入会更高、毛利率可能更低；若以芯片收入为主，收入较低但毛利率更好。本模型将2027毛利率控制在55.5%，没有同时假设“完整机架收入”和“纯芯片毛利率”。",
    )
    ...
    add_text(
        doc,
        "AMD为无晶圆厂模式，资本开支强度较低，盈利增长可转化为较强自由现金流。但系统业务放量会提高存货、应收和客户融资需求；同时50亿美元战略投资、NeoCloud支持及潜在回购会限制现金余额的机械累积。",
    )
    ...
    add_text(
        doc,
        "ROCm.AI与Coding Agent的价值是真实的：它们可帮助代码转换、算子适配和性能调优，降低首次迁移的人力成本。但生产系统的选择由全生命周期TCO决定，不能只看示范代码是否运行。",
    )
    add_table(
        doc,
        ["层面", "ROCm改善方向", "尚待验证"],
    ...
    add_source(
        doc, "资料来源：本报告测算。目标价值未额外加入净现金溢价，以避免对潜在投资和营运资金形成双重乐观。"
    )
    ...
    add_text(
        doc,
        "640美元基本等于18.28美元2027E EPS乘35倍。其数学没有问题，但把一个接近乐观情景的盈利预测包装为基准情景。若采用14.1美元基准EPS，即使给35倍，价值也只有约494美元。目标价争议的核心是盈利兑现，而不是2-3倍PE差异。",
    )
    ...
    for title, body in [
        ...
        add_rich_para(
            doc,
            [
                (title, {"size": 10.3, "bold": True, "color": BLUE}),
                (body, {"size": 10.3, "bold": False, "color": INK}),
            ],
            after=6,
        )
    ...
    for title, body in risks:
        ...
        add_rich_para(
            doc,
            [
                (title, {"size": 10.2, "bold": True, "color": color}),
                ("  " + body, {"size": 10.2, "bold": False, "color": INK}),
            ],
            after=7,
        )
    ...
    add_text(
        doc,
        "本报告以AMD 2025财年实际数据、2026Q1实际数据和2026Q2公司指引为历史锚。2026-2028年均为研究情景，不是公司指引。Non-GAAP指标用于跨期经营比较；估值同时考虑潜在权证摊薄，不把最高权证股数机械视为必然归属。",
    )
