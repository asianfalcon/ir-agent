...
def _company_from_vocab(ticker: str, fallback_name: str = "") -> Company:
    vocab = _load_json(SETTINGS.vocab_path, [])
    for entry in vocab:
...
def _watchlist_companies() -> list[Company]:
    tickers = _load_json(SETTINGS.config_root / "watchlist.json", [])
...
def fetch_financials(company: Company) -> list[dict]:
    ...
    out_dir = SETTINGS.external_source_root / "financials" / "akshare"
    ...
    for name, func, kwargs in jobs:
        try:
            ...
            results.append({"name": name, "rows": len(records), "path": SETTINGS.locator(path)})
...
def update_sqlite_financial_supplements(company: Company) -> int:
    ...
    candidates = sorted((SETTINGS.external_source_root / "financials" / "akshare").glob(
        f"{company.ticker}_financial_indicator_report_*.json"
    ))
    if not candidates:
        return 0
...
def fetch_announcements(company: Company, start_date: str) -> dict:
    ...
    today = date.today().isoformat().replace("-", "")
    raw_dir = SETTINGS.external_source_root / "announcements" / "akshare"
    processed_dir = SETTINGS.derived_root / "announcements" / "akshare" / company.ticker
...
def fetch_news(company: Company, limit: int) -> dict:
    ...
    today = date.today().isoformat().replace("-", "")
    raw_dir = SETTINGS.external_source_root / "news" / "akshare"
    processed_dir = SETTINGS.derived_root / "news" / "akshare" / company.ticker
    raw_dir.mkdir(parents=True, exist_ok=True)
    processed_dir.mkdir(parents=True, exist_ok=True)
...
def report_files(company: Company) -> list[Path]:
    paths = sorted((SETTINGS.manual_source_root / "reports" / company.name).rglob("*.pdf"))
...
def gc_stale_report_rows(ticker: str, current_files: list[Path]) -> int:
    ...
    current = {SETTINGS.locator(p) for p in current_files}
    db = lancedb.connect(str(SETTINGS.lance_path))
